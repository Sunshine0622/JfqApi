...
class  Ceshi extends CI_Controller {
		...
		if($rearr['adid']==1){
			...
			if($list['api_cat']==1){
				...
				if($TrueValue=='auto'){
					...
					echo  json_encode(array('code'=>'0','result'=>'ok','message'=>$file_contents));die; //这里返回0代表成功
				}else{
					if($KCount==1){
				    	if($json[$keys[0]]==$TrueValue){
							...
							echo  json_encode(array('code'=>'103','result'=>'false','message'=>$file_contents));die; 
				    	}
				    }else if($KCount==2){
			...
			if($list['api_cat']==1){
				...
				$this->db->insert('aso_IdfaRepeat_log',$log);
			 	if($KCount==1){
			 		if($nTrueValue==null){
			 			if($json[$keys[0]]==$TrueValue){
			    			echo json_encode(array($rearr['idfa']=>'1','message'=>$file_contents));die;
			    		}else{
			    			echo json_encode(array($rearr['idfa']=>'0','message'=>$file_contents));die;
			    		}
			 		}else{
			...
			if($list['api_cat']==1){
				...
				if($TrueValue=='auto'){
			    	...
			    	if($json[$keys[0]]==$TrueValue){
						...
						echo  json_encode(array('code'=>'0','result'=>'ok','message'=>$file_contents));die;
	...
	function IdfaRepeat_12($data,$list){
		...
		$this->db->insert('aso_IdfaRepeat_log',$log);
		//渠道返回：成功返回0  失败返回1
		if(!$json[$data['idfa']] && isset($json[$data['idfa']])){
			//成功返回
			echo  json_encode(array($data['idfa']=>'1'));//这里返回1代表成功
		}else{
			$a = array($data['idfa']=>'0');//我们这里返回0代表失败不可做任务
	...
	function source_16($data,$list){
		...
		$url = $list['source_url']."&idfa=".$data['idfa']."&ip=".$data['ip']."&keyword=".urlencode($data['keywords']).'&os='.$data['os'];
	...
	function submit_16($data,$list){
		$file_contents = $this->request_get1($list['submit_url'] . "&idfa=" . $data['idfa']."&ip=".$data['ip'].'&keyword='.urlencode($data['keywords']));
		$json = json_decode($file_contents,true );
	...
	function source_33($data,$list){
		...
		$callback = urlencode($callback);
		$url = $list['source_url']."&idfa=".$data['idfa']."&ip=".$data['ip']."&os_version=".$data['os']."&mac=02:00:00:00:00:00&callback=".$callback;
