...
class  Mapi extends CI_Controller {
	...
	public function aso_cesi(){
		//$this->mem->add("1258460288_10261", array('appid'=>1258460288, 'cpid'=>10281, "ccc", "ddd"),0,5); //存数组
		print_r($this->mem->get("1258460288_10261"));
	...
	public function aso_source(){
		...
		// if($list['is_source']==0){
		// 	echo json_encode(array('code'=>107,'message'=>'Source is closed'));die;
		// }
		...
		if($rearr['adid']==1){
			...
			if($list['api_cat']==1){
				...
				if($error){
					...
					$this->db->update('aso_error',array('source_request_count'=>$NowReCo+1),array('appid'=>$rearr['appid'],'adid'=>$rearr['adid']));
				}else{
				...
				if($TrueValue=='auto'){
					...
					if($KCount==1){
				    	if($json[$keys[0]]==$TrueValue){
							...
							$this->db->update('aso_error',array('source_error_count'=>$error['source_error_count']+1),array('appid'=>$rearr['appid'],'adid'=>$rearr['adid']));
	...
	public function aso_advert(){
		...
		$sql = "select adid,cpid,appid,idfa,callback from aso_source where appid=".$callarr['appid']." and idfa='".$callarr['idfa']."'" . " ORDER BY id DESC LIMIT 1";//
		// echo $sql;die;
		$res = $this->db->query($sql);
		$list = $res->row_array();
		if(empty($list)){
 	...
 	// 	$respone       = $file_contents;
 	// 	$logInfo       = 'ImoneyUrl:'.$request_url.'---ClientUrl:'.$url.'---ClientRespone:'.$respone.'---渠道cpid:'.$list['cpid'];
 	// 	$this->log($logInfo);
		//给你花特殊处理
		if($callarr['appid']==1076275012 && $list['adid']==10042){
			if($list['cpid']!=207){
				$this->db->update('aso_source',array('type'=>2,'activetime'=>time()),array('appid'=>$list['appid'],'cpid'=>$list['cpid'],'adid'=>$list['adid'],'idfa'=>$list['idfa']));
		...
		//腾讯动漫特殊处理
		if($callarr['appid']==569387346 && $list['adid']==10018){
	...
	public function aso_IdfaRepeat(){
		...
		// if($list['is_repeat']==0){
		// 	echo json_encode(array('code'=>107,'message'=>'Repeat is closed'));die;
		// }
		//判断是否是https请求
		...
		if($rearr['adid']==1){
			...
			if($list['api_cat']==1){
			    ...
			    // $locking_data = $this->db->query($locking_sql)->row_array();
				...
				if($error){
					...
					$this->db->update('aso_error',array('repeat_request_count'=>$NowReCo+1),array('appid'=>$rearr['appid'],'adid'=>$rearr['adid']));
				}else{
				...
				$this->db->insert('aso_IdfaRepeat_log',$log);
			 	if($KCount==1){
			 		if($nTrueValue==null){
			 			if($json[$keys[0]]==$TrueValue){
			 				//$this->db->insert('aso_task_locking',$log);
			    			echo json_encode(array($rearr['idfa']=>'1'));die;
			    		}else{
			    			$this->db->update('aso_error',array('repeat_error_count'=>$error['repeat_error_count']+1),array('appid'=>$rearr['appid'],'adid'=>$rearr['adid']));
	...
	public function aso_Submit(){
		...
		// if($list['is_submit']==0){
		// 	echo json_encode(array('code'=>107,'message'=>'Submit is closed'));die;
		// }
		...
		//如果厂商的cpid为1，就是我们自己来做排重只对接渠道，不对接厂商，只做排重点击上报
			if(empty($Soresult)){//等于空就是没有做任务，返回错误值
				...
				$this->db->update('aso_source',array('type'=>1,'activetime'=>time()),array('appid'=>$rearr['appid'],'cpid'=>$rearr['cpid'],'adid'=>$rearr['adid'],'idfa'=>$rearr['idfa']));
			...
			if($list['api_cat']==1){
				...
				if($error){
					...
					$this->db->update('aso_error',array('submit_request_count'=>$NowReCo+1),array('appid'=>$rearr['appid'],'adid'=>$rearr['adid']));
				}else{
				...
				if($TrueValue=='auto'){
			    	...
			    	if($json[$keys[0]]==$TrueValue){
						...
						$rearr['json'] =$file_contents;
						$this->db->insert('aso_submit_log',$rearr);
						$this->db->update('aso_error',array('submit_error_count'=>$error['submit_error_count']+1),array('appid'=>$rearr['appid'],'adid'=>$rearr['adid']));
			    		echo  json_encode(array('code'=>'103','result'=>'false'));die;
			    	}
			    }else if($KCount==2){
			    	if($json[$keys[0]][$keys[1]]==$TrueValue){
	...
	function submit_3($data,$list){
		...
		if(isset($json['status']) && $json['status']==1){//上报成功
			...
			$data['type'] = 1;
		       ...
		       if(!empty($SourceExists)){
				     $this->db->update('aso_source',array('type'=>1,'activetime'=>time()),array('appid'=>$data['appid'],'cpid'=>$data['cpid'],'adid'=>$data['adid'],'idfa'=>$data['idfa']));
		        }else{
   ...
   //爱盈利-1上报
	...
	function IdfaRepeat_10265($data,$list){
		...
		if(isset($json['responseBody']) && $json['responseBody']=='{"isRepeat":"N"}'){
			...
			echo  json_encode(array($data['idfa']=>'0'));die; //这里返回1代表成功
		}else{
			 echo $file_contents;
		}
	}
	...
	function IdfaRepeat_10268($data,$list){
		...
		if(isset($json['result'][$data['idfa']]) && $json['result'][$data['idfa']]==0){
			...
			echo  json_encode(array($data['idfa']=>'0'));exit(); //这里返回1代表成功
		}else{
			$a = array($data['idfa']=>'0'); //我们这里返回0代表失败不可做任务
			$contents = json_encode($a);
			echo  $contents;exit();
...
// echo $source.'|'.$adid.'|'.$data['idfa'].'|'.'m5f5nohv4mk5gz2pa7h9hr5qrd9mv14b'.'|'.$data['timestamp'];
		...
		//渠道返回：成功返回0  失败返回1
		if(isset($json['data'][1]) && $json['data'][1]==0){
			echo  json_encode(array($data['idfa']=>'1'));die; //这里返回1代表成功
		}else{
			$a = array($data['idfa']=>'0'); //我们这里返回0代表失败不可做任务
			$contents = json_encode($a);
