...
class  Api extends CI_Controller {
	...
	public function aso_IdfaRepeat(){
		...
		if($rearr['adid']==1){
			...
			/*判断是否在本地激活库里*/
			// $ActiveData=  $this->db->get_where('aso_source',array('appid'=>$rearr['appid'],'idfa'=>$rearr['idfa'],'type'=>1))->row_array();
			// if($ActiveData){
	...
	public function aso_Submit(){
		...
		//如果厂商的cpid为1，就是我们自己来做排重只对接渠道，不对接厂商，只做排重点击上报
			...
			if($list['submit_type']==2){
				...
				$SourceExists=  $this->db->get_where('aso_source',array('appid'=>$rearr['appid'],'idfa'=>$rearr['idfa'],'type'=>1))->row_array();
	...
	function source_103($data,$list){
		...
		$url=$list['source_url']."&idfa=".$data['idfa']."&ip=".$data['ip']."&callback=".$callback."&os=".$data['os'].'&keyword='.$data['keywords'];
		$file_contents   = $this->request_get($url);
		$json  = json_decode($file_contents,true );
		//print_r($url);die;
		//print_r($json);exit;
